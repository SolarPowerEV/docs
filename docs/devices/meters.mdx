...
Der elgris SMART METER LAN ist vollständig kompatibel zu SMA. Insofern ist - bis jemand ein eigenes Template für ihn geschrieben hat - die einfachste Option ihn für evcc zu nutzen, die Einbindung über das bereits vorhandene SMA Energy Meter Template. Er bietet grundsätzlich neben der Verbindung über TCP/IP und einem Webinterface auch noch bis zu drei Modbus-Anbindungen gleichzeitig an. Zur Nutzung in evcc mit dem SMA-Template muss man im Webinterface das Setting "SMA METER INTERVAL" auf mindestens 1000 ms (oder kürzer) setzen. Ansonsten meldet evcc den FATAL ERROR: "cannot create meter 'grid': cannot create meter type 'template': cannot create meter type 'sma': failed to get device: no Speedwire ping response for 192.0.2.2"
...
Es wird dringend empfohlen die Verbindung über einen LSE-3 Logger Stick herzustellen. Andere Verbindungswege (z.B. LSW-3 oder via RS485) sind u.U. instabil und benötigen Verzögerungen (delay) > 0s.
